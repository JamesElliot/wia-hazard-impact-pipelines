...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(description="Run standardized SPEI post-run checks (schema + parity).")
    p.add_argument("--run-dir", required=True, help="Path to SPEI run directory.")
    p.add_argument(
        "--skip-metadata-validation",
        action="store_true",
        help="Skip run_metadata schema validation.",
    )
    p.add_argument(
        "--strict-metadata-validation",
        action="store_true",
        help="Fail if schema validation fails (default: warning only).",
    )
    p.add_argument(
        "--skip-parity",
        action="store_true",
        help="Skip parity report generation.",
    )
    return p
